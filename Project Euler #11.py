...
array = [s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15, s16, s17, s18, s19, s20]

for i in range(0, len(array)):
	array[i] = list(array[i].split(' '))
...
for i in range(0,len(array)):
        for j in range(0, len(array[i])):
                try:
                       products.append(array[i][j]*array[i+1][j]*array[i+2][j]*array[i+3][j])#vertical
                       cells.append([i,j])
                except:
                        pass

                try:
                       products.append(array[i][j]*array[i][j+1]*array[i][j+2]*array[i][j+3])#horizontal
                       cells.append([i,j])
                except:
                        pass

                try:
                       products.append(array[i][j]*array[i+1][j+1]*array[i+2][j+2]*array[i+3][j+3])#diagonal
                       cells.append([i,j])
                except:
                        pass

                try:
                       products.append(array[i][j]*array[i+1][j-1]*array[i+2][j-2]*array[i+3][j-3])#vertical
                       cells.append([i,j])
                except:
                        pass
...
print(max(products))
print(cells[products.index(max(products))])
